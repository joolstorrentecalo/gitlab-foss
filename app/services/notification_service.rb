...
class NotificationService
  ...
  def repository_rewrite_history_success(project, user)
    ...
    mailer.send(:repository_rewrite_history_success_email, project, user).deliver_later
  end

  def repository_rewrite_history_failure(project, user, error)
    return if project.emails_disabled?

    mailer.send(:repository_rewrite_history_failure_email, project, user, error).deliver_later
  end
