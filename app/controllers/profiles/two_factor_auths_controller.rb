...
class Profiles::TwoFactorAuthsController < Profiles::ApplicationController
  ...
  def destroy_otp
    # If no WebAuthn devices we can destroy the whole two-factor authentication service
    return destroy if current_user.webauthn_registrations.empty?

    # Otherwise, we destroy just the otp authenticator
    result = TwoFactor::DestroyOtpService.new(current_user, user: current_user).execute

    if result[:status] == :success
      redirect_to profile_account_path, status: :found, notice: _('One-time password authenticator has been deleted!')
    else
      redirect_to profile_account_path, status: :found, alert: result[:message]
    end
  end
