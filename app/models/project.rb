...
class Project < ApplicationRecord
  ...
  def log_import_activity(job_id, type: :import)
    ...
    if job_id
      using_primary = ::Gitlab::Database::LoadBalancing::Session.current.use_primary?(connection.load_balancer.name)
      Gitlab::AppLogger.info("#{job_type} job scheduled for #{full_path} with job ID #{job_id} (primary: #{using_primary}).")
    else
