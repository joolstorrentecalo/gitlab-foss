...
module Gitlab
  module BackgroundMigration
    class RestoreOptInToGitlabCom < BatchedMigrationJob
      ...
      scope_to ->(relation) { relation.where.not('onboarding_status::jsonb ? :key', key: 'email_opt_in') }
      ...
      def perform
        each_sub_batch do |sub_batch|
          sub_batch.connection.execute(
            construct_query(
              sub_batch.joins(
                "INNER JOIN #{temporary_table_name} on user_id = #{temporary_table_name}.GITLAB_DOTCOM_ID"
              )
            )
          )
        end
