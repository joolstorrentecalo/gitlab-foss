  ...
  module Database
    module PartitioningMigrationHelpers
      module TableManagementHelpers
        ...
        #   :partitioned_table_name - the name for the routing table. If not specified, a default is generated
        #
        ...
        def partition_table_by_date(table_name, column_name, min_date: nil, max_date: nil, partitioned_table_name: nil)
          Gitlab::Database::QueryAnalyzers::RestrictAllowedSchemas.require_ddl_mode!
        ...
        #     partition_name_format: "p_%{partition_name}_%{table_name}",
        #     create_partitioned_table_fn: -> { create_custom_partitioned_table }
        #
        # Options are:
        #   :primary_key - a array specifying the primary query of the new table
        #   :partitioned_table_name - the name for the routing table. If not specified, a default is generated
        ...
        def partition_table_by_list(
          ...
          primary_key:, partitioned_table_name: nil, partition_name_format: nil, partition_mappings: nil,
        ...
        #
        def drop_partitioned_table_for(table_name, partitioned_table_name: nil)
          assert_table_is_allowed(table_name)
          assert_not_in_transaction_block(scope: ERROR_SCOPE)
        ...
        #
        def enqueue_partitioning_data_migration(table_name, partitioned_table_name: nil)
          assert_table_is_allowed(table_name)
        ...
        #
        def cleanup_partitioning_data_migration(table_name, partitioned_table_name: nil)
          assert_table_is_allowed(table_name)
        ...
        def create_partitioned_copy(
          ...
          return create_partitioned_table_fn.call if create_partitioned_table_fn.is_a?(Proc)
        ...
        def create_sync_function(name, source_table_name, partitioned_table_name, unique_key)
          ...
          delimiter = ",\n    "
          source_column_names = connection.columns(source_table_name).map(&:name)
          partition_column_names = connection.columns(partitioned_table_name).map(&:name)
          column_names = partition_column_names.intersection(source_column_names)
