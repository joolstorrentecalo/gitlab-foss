...
RSpec.describe Gitlab::Import::UsernameMentionRewriter, feature_category: :importers do
  ...
  describe '#update_text_username_mentions' do
    context 'when text is nil' do
      it 'returns nil' do
        expect(instance.update_text_username_mentions(nil)).to be_nil
      end
    end

    context 'when the text is empty' do
      it 'returns an empty string' do
        expect(instance.update_text_username_mentions('')).to eq('')
      end
    end
    ...
    context 'when the text contains username mentions' do
      ...
      it 'wraps them in backticks preserving punctuation' do
        expect(instance.update_text_username_mentions(original_text)).to eq(expected_text)
      end
    end
