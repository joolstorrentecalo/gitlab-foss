...
RSpec.describe Gitlab::Database::PartitioningMigrationHelpers::TableManagementHelpers, feature_category: :database do
  ...
  describe '#partition_table_by_date' do
    ...
    describe 'constructing the partitioned table' do
      ...
      context 'when partitioned_table_name is specified' do
        let(:specific_partitioned_table) { '_test_gitlab_main_p_specific_table' }

        it 'uses the specified table name', :aggregate_failures do
          migration.partition_table_by_date(
            source_table, partition_column, min_date: min_date, max_date: max_date,
            partitioned_table_name: specific_partitioned_table
          )

          expect_range_partitions_for(partitioned_table, {})
          expect_range_partitions_for(specific_partitioned_table, {
            '000000' => ['MINVALUE', "'2019-12-01 00:00:00'"],
            '201912' => ["'2019-12-01 00:00:00'", "'2020-01-01 00:00:00'"],
            '202001' => ["'2020-01-01 00:00:00'", "'2020-02-01 00:00:00'"],
            '202002' => ["'2020-02-01 00:00:00'", "'2020-03-01 00:00:00'"],
  ...
  describe '#partition_table_by_list' do
    ...
    context 'simulates the ci_runners migration' do
      ...
      context 'when create_partitioned_table_fn is specified' do
        ...
        let(:opts) do
          { partition_mappings: partitions, create_partitioned_table_fn: -> { create_partitioned_table } }
        end
        ...
        def create_partitioned_table
          migration.create_table partitioned_table, primary_key: new_primary_key, options: partitioned_table_opts do |t|
  ...
  describe '#drop_partitioned_table_for' do
    ...
    context 'with partitioned_table_name specified' do
      let(:specific_partitioned_table) { '_test_gitlab_main_p_specific_table' }
      let(:parent_table) { specific_partitioned_table }

      it 'used specified table name' do
        migration.partition_table_by_date(
          source_table, partition_column, min_date: min_date, max_date: max_date,
          partitioned_table_name: specific_partitioned_table
        )
        ...
        migration.drop_partitioned_table_for(source_table, partitioned_table_name: specific_partitioned_table)
    ...
    context 'when records exist in the source table' do
      ...
      context 'when partitioned_table_name is specified' do
        ...
        it 'enqueues jobs to copy each batch of data' do
          ...
          Sidekiq::Testing.fake! do
            migration.enqueue_partitioning_data_migration(
              source_table, partitioned_table_name: specific_partitioned_table)

            expect(migration_class).to have_scheduled_batched_migration(
              table_name: source_table,
              column_name: :id,
              job_arguments: [specific_partitioned_table],
              batch_size: described_class::BATCH_SIZE,
              sub_batch_size: described_class::SUB_BATCH_SIZE
            )
          end
  ...
  describe '#cleanup_partitioning_data_migration' do
    ...
    context 'when tracking records exist in the batched_background_migrations table' do
      ...
      context 'when partitioned_table_name is specified' do
        context 'with the same table name as the one in the migration' do
          let(:specific_partitioned_table) { '_test_gitlab_main_p_specific_table' }
          let(:partitioned_table) { specific_partitioned_table }
        ...
        context 'with a different table name than the one in the migration' do
          let(:specific_partitioned_table) { '_test_gitlab_main_p_specific_table' }

          it 'does not delete those pertaining to the given table' do
            expect do
              migration.cleanup_partitioning_data_migration(source_table, partitioned_table_name: specific_partitioned_table)
