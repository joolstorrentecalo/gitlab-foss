...
RSpec.describe Gitlab::BackgroundMigration::FixPickUpAtCiDeletedObject, schema: 20240912122513, migration: :gitlab_ci, feature_category: :job_artifacts do
  ...
  describe '#perform' do
    context 'when there are invalid records' do
      it 'resets pick_up_at values' do
        expect { migration.perform }.to not_change { deleted_object1.reload.pick_up_at }

        expect(deleted_object2.reload.pick_up_at).to be_within(2.seconds).of(1.hour.from_now)
