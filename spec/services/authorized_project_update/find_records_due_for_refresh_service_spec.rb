...
RSpec.describe AuthorizedProjectUpdate::FindRecordsDueForRefreshService, feature_category: :groups_and_projects do
  ...
  where(:feature_enabled) do
    [true, false]
  end

  with_them do
    before do
      stub_feature_flags(improve_find_records_for_refresh_service: feature_enabled)
    end
    ...
    describe '#needs_refresh?' do
      subject(:needs_refresh) { service.needs_refresh? }
    ...
    describe '#current_authorizations_per_project' do
      ...
      it 'sets the values to access level' do
        expect(hash.values.length).to eq(1)

        value = hash.values[0]

        expect(value).to eq(Gitlab::Access::OWNER)
      end
    end
