...
RSpec.describe User, feature_category: :user_profile do
  ...
  describe '#verified_detumbled_emails' do
    ...
    it 'returns only confirmed unique detumbled emails' do
      create(:email, email: 'user+2@example.com', user: user, confirmed_at: Time.current)
      create(:email, email: 'other_user+1@example.com', user: user, confirmed_at: Time.current)
      create(:email, user: user)
