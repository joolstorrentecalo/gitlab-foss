...
RSpec.describe Issuable, feature_category: :team_planning do
  ...
  describe '.iid_prefix_search' do
    let_it_be(:project) { create(:project) }

    let_it_be(:issue_1) { create(:issue, project: project, iid: 1) }
    let_it_be(:issue_10) { create(:issue, project: project, iid: 10) }
    let_it_be(:issue_20) { create(:issue, project: project, iid: 20) }
    let_it_be(:issue_21) { create(:issue, project: project, iid: 21) }
    let_it_be(:issue_135) { create(:issue, project: project, iid: 135) }
    let_it_be(:issue_138) { create(:issue, project: project, iid: 138) }
    let_it_be(:issue_19999999) { create(:issue, project: project, iid: 199999999) }

    it 'returns issuables that match the given iid' do
      expect(issuable_class.iid_prefix_search('21')).to contain_exactly(issue_21)
    end

    it 'returns issuables with iids starting with the given prefix' do
      expect(issuable_class.iid_prefix_search('1')).to contain_exactly(issue_1, issue_10, issue_135, issue_138, issue_19999999)
    end

    it 'returns issuables with iids starting with the given 2-digit prefix' do
      expect(issuable_class.iid_prefix_search('13')).to contain_exactly(issue_135, issue_138)
    end
  end
