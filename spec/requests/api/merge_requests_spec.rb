...
RSpec.describe API::MergeRequests, :aggregate_failures, feature_category: :source_code_management do
  ...
  describe "PUT /projects/:id/merge_requests/:merge_request_iid/merge", :clean_gitlab_redis_cache do
    ...
    context 'when merge_when_checks_pass is off' do
      ...
      it "does not enable auto merge if not mergeable only_allow_merge_if_pipeline_succeeds is true" do
      ...
      context 'when the pipeline failed' do
        ...
        it "does not enable auto merge if the pipeline is failed and only_allow_merge_if_pipeline_succeeds is true" do
          allow_any_instance_of(MergeRequest).to receive_messages(head_pipeline: pipeline, diff_head_pipeline: pipeline)
          project.update_attribute(:only_allow_merge_if_pipeline_succeeds, true)
    ...
    it "enables auto merge if not mergeable only_allow_merge_if_pipeline_succeeds is true" do
      allow_any_instance_of(MergeRequest)
        .to receive_messages(
          head_pipeline: pipeline,
          diff_head_pipeline: pipeline
