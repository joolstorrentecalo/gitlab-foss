...
RSpec.shared_examples 'handling get metadata requests' do |scope: :project|
  ...
  shared_examples 'handling all conditions' do
    ...
    context 'when unauthenticated', if: scope == :project || scope == :group do
      let(:headers) { {} }

      context 'with public project' do
        before do
          set_visibility('public', scope)
        end

        it_behaves_like 'accept metadata request', status: :ok
      end

      context 'with non-public project' do
        before do
    ...
    context 'when authenticated', if: scope == :project || scope == :group do
    ...
    context 'with instance scope', if: scope == :instance do
      ...
      it_behaves_like 'redirect metadata request', status: :redirected
    end
  end
  ...
  context 'when the package does not exist' do
    ...
    context 'with group scope', if: scope == :group do
      context 'when request forwarding is enabled' do
        before do
          set_npm_package_requests_forwarding(true, :instance)
        end

        it_behaves_like 'redirect metadata request', status: :redirected
      end

      context 'when request forwarding is disabled' do
        before do
          set_npm_package_requests_forwarding(false, :instance)
        end

        context 'with a private group and unauthenticated user' do
          let(:headers) { {} }
