...
module Tooling
  module Danger
    module StableBranch
      ...
      TEMPLATE_SOURCE_REGEX = %r{<!--\s*template\s+sourced\s+from\s+(https?://\S+)\s*-->}i
      ...
      **The `e2e:test-on-omnibus-ee` job needs to succeed or have approval from a Software Engineer in Test.**
      ...
      This MR does not appear to be using the [stable branch template](https://gitlab.com/gitlab-org/gitlab/-/blob/master/.gitlab/merge_request_templates/Stable%20Branch.md).
      Please update the description to use the correct template.
      MSG
      ...
      def template_source_url
        helper.mr_body.scan(TEMPLATE_SOURCE_REGEX).flatten.first
      end

      def uses_stable_branch_template?
        template_source_url == 'https://gitlab.com/gitlab-org/gitlab/-/blob/master/.gitlab/merge_request_templates/Stable%20Branch.md'
      end
    end
  end
end
